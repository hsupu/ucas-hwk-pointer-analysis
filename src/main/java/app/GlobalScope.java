...
public class GlobalScope implements Cloneable{
    ...
    public Integer getAllocId() {//alloc()
        return allocId;
    }
    ...
    public void addQuery(int id, Local local) {//test()
        queries.put(id, local);
    }

    public void add(Local local, VarBox vb) {// save Local and its VarBox to vbMap
        vbMap.put(local, vb);
    }
    ...
    public String toString(){
	    ...
	    for (Map.Entry<Local, VarBox> entry : this.vbMap.entrySet()){
	    	mapStr.add(entry.getKey().toString() + " = " + entry.getValue().toString()+/*" "+System.identityHashCode(entry.getValue())+*/"\n");
	    }
	    if(mapStr.size()>0){
